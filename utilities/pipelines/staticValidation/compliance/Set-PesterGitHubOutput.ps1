...
function Set-PesterGitHubOutput {
    ...
    $moduleSplitRegex = '[\/|\\]elements[\/|\\](res|ptn|utl)[\/|\\]'
    ...
    if ($failedTests.Count -gt 0) {
        ...
        foreach ($failedTest in ($failedTests | Sort-Object -Culture 'en-US' -Property { $PSItem.ExpandedName })) {
            ...
            if ($failedTest.ScriptBlock.File -match $moduleSplitRegex) {
                ...
                $testFile = ('elements/{0}/{1}' -f $testFileIdentifier[1], $testFileIdentifier[2]) -replace '\\', '/' # e.g., [avm\res\cognitive-services\account\tests\unit\custom.tests.ps1]
            } else {
                # None-module test
