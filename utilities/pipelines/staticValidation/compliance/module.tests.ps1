...
Describe 'File/folder tests' -Tag 'Modules' {
    ...
    Context 'General module folder tests' {
        ...
        foreach ($moduleFolderPath in $moduleFolderPaths) {
            $null, $moduleType, $resourceTypeIdentifier = ($moduleFolderPath -split '[\/|\\]elements[\/|\\](res|ptn|utl)[\/|\\]') # 'avm/res|ptn|utl/<provider>/<resourceType>' would return 'avm', 'res|ptn|utl', '<provider>/<resourceType>'
        ...
        foreach ($moduleFolderPath in $moduleFolderPaths) {
            ...
            if (($resourceTypeIdentifier -split '[\/|\\]').Count -eq 2) {
                $topLevelModuleTestCases += @{
                    moduleFolderName = $moduleFolderPath.Replace('\', '/').Split('/elements/')[1]
                    moduleFolderPath = $moduleFolderPath
                    moduleType       = $moduleType
                }
            }
...
#         $resourceTypeIdentifier = ($moduleFolderPath -split '[\/|\\]elements[\/|\\](res|ptn|utl)[\/|\\]')[2] -replace '\\', '/' # 'avm/res|ptn|utl/<provider>/<resourceType>' would return '<provider>/<resourceType>'
#         $relativeModulePath = Join-Path 'avm' ($moduleFolderPath -split '[\/|\\]elements[\/|\\]')[1]
    ...
    Context 'Template tests' -Tag 'Template' {
        ...
        Context 'Parameters' {
            ...
            Context 'Schema-based User-defined-types tests' -Tag 'UDT' {
                ...
                foreach ($moduleFolderPath in $moduleFolderPaths) {
                    ...
                    if ($moduleFolderPath -match '[\/|\\]elements[\/|\\](ptn|utl)[\/|\\]') {
        ...
        Context 'Output' {
            ...
            It '[<moduleFolderName>] Location output should be returned for resources that use it.' -TestCases $moduleFolderTestCases {
                ...
                $primaryResourceType = (Split-Path $TemplateFilePath -Parent).Replace('\', '/').split('/elements/')[1]
