...
function Set-ModuleFileAndFolderSetup {
    ...
    if ([String]::IsNullOrEmpty($CurrentLevelFolderPath)) {
        ...
        $repoRoot, $moduleType, $resourceTypeIdentifier = $FullModuleFolderPath -split '[\/|\\]elements[\/|\\](res|ptn|utl)[\/|\\]' # .*/bicep-registry-modules, res|ptn|utl, <provider>/<resourceType>
        ...
        $CurrentLevelFolderPath = Join-Path $repoRoot 'elements' $moduleType $providerNamespace $resourceType
    }

    # Collect data
    $resourceTypeIdentifier = ($CurrentLevelFolderPath -split '[\/|\\]elements[\/|\\](res|ptn|utl)[\/|\\]')[2] # avm/res/<provider>/<resourceType>
